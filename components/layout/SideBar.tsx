...
const sideBarLinks = [
  ...
  },
  {
    title: "My Issues",
    icon: MessageSquare,
    link: "/user/dashboard/issues"
  },
  {
    title: "Social Media Audits",
    icon: Star,
    link: "/user/dashboard/audits"
  },
  {
...
export default function SideBar({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex flex-col max-h-[calc(100vh-70px)] justify-between bg-white border-r">
          ...
          <div className="flex-1 flex-col gap-3 overflow-auto py-2">
            <nav className="flex flex-col gap-5 px-4 text-sm font-medium">
              {sideBarLinks.map((item, index) => {
                const isOverview = item.link === "/admin";
